...
mysql -uroot -p"${MYSQL_ROOT_PASSWORD}" <<-EOSQL
  ...
  CREATE USER IF NOT EXISTS 'warehouse_user'@'%' IDENTIFIED WITH caching_sha2_password BY '${WAREHOUSE_DB_PASSWORD}';
  ...
  GRANT ALL PRIVILEGES ON warehouse.* TO 'warehouse_user'@'%';
  ...
  GRANT ALL PRIVILEGES ON warehouse.* TO '${MYSQL_USER}'@'%';
  FLUSH PRIVILEGES;
EOSQL
